...
def synthesize_with_elevenlabs(text, voice_id, api_key):
    ...
    if response.status_code == 200:
        filename = f"/tmp/elevenlabs_{uuid.uuid4()}.mp3"
...
async def voice(request: Request):
    try:
        ...
        audio_filename = synthesize_with_elevenlabs(response_text, ELEVENLABS_VOICE_ID, ELEVENLABS_API_KEY)
        audio_basename = os.path.basename(audio_filename)
        audio_url = f"{os.getenv('NGROK_URL')}/audio/{audio_basename}"

        # Respond with <Play> TwiML
        response.play(audio_url)

        # Gather for the next turn
        gather = Gather(
            input="speech",
            action="/voice",
            method="POST",
        ...
        gather.say("You can speak after the beep.", voice="Polly.Joanna-Neural", language="en-US")
        response.append(gather)
        response.redirect("/voice")
        return Response(str(response), media_type="application/xml")
    except Exception as e:
